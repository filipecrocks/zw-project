...
//
// Usa uma chave de embeddings do Google: GEMINI_API_KEY (crie a sua no
// Google AI Studio). O Groq só faz geração/LLM — não serve embeddings; por isso
// a parte vetorial do ZW usa o modelo de embedding do Google.
//
...
if (!SUPABASE_URL) fail('Falta SUPABASE_URL (a URL do seu projeto Supabase, ex.: https://xxxx.supabase.co).');
if (!SERVICE_ROLE_KEY) fail('Falta SUPABASE_SERVICE_ROLE_KEY (a chave secreta/service_role do seu projeto Supabase).');
if (!GEMINI_API_KEY) fail('Falta GEMINI_API_KEY (a sua chave do Google AI Studio).');
