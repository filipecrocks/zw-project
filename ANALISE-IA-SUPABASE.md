...
> Escopo: **só** o banco do ZW (o seu projeto Supabase).
> Pipeline de IA: embeddings `gemini-embedding-001` (1536 dims, casa com a coluna
> existente) e geração Groq `llama-3.3-70b-versatile`, com as suas próprias chaves.
> Não toca na Arena nem no fluxo de convidado.
...
- **Chunk:** **1 chunk por item** (itens têm ~35 tokens — muito abaixo de qualquer
  ...
  ~2.800 chars com 480 de overlap (params típicos de chunking).
- **Reembeddar só no que muda:** guardar um hash do conteúdo (um campo
  `conteudo_hash`) e regenerar o embedding só quando o texto da fonte mudar. O job
...
- **Limite proposto:** rate limit por sessão (15 msgs/h + cooldown 3s, padrão de
  chat público) + teto de tamanho da pergunta (~1.000 chars).
...
- **Rate limiting:** a função `chat` **ainda não tem** — recomendo criar
  uma tabela `portal_rate_limit` (por `session_id`: 15 msgs/h + cooldown 3s). Importante
